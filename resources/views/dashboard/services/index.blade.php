...
@section('content')
                ...
                <div class="card hover:shadow-xl transition-all duration-300">
                    ...
                    @if (auth()->user()->isAdmin())
                        <div class="flex space-x-2 pt-4 border-t border-gray-200">
                            <button type="button"
                                onclick="openEditServiceModal({{ $service->id }}, '{{ $service->name }}', {{ $service->category_id }}, {{ $service->duration_minutes }}, {{ $service->price_regular }}, {{ $service->price_premium ?? 'null' }}, '{{ addslashes($service->description) }}', {{ $service->is_premium ? 'true' : 'false' }})"
                            ...
                            <form action="{{ route('dashboard.services.destroy', $service) }}" method="POST"
                                ...
                                @csrf
                                @method('DELETE')
                                <button type="submit" onclick="return confirmServiceDelete(event)"
                                    class="w-full bg-red-600 hover:bg-red-700 text-white py-2 px-4 rounded-lg transition text-sm font-semibold">
    ...
    <script>
        ...
        function openEditServiceModal(id, name, categoryId, duration, priceRegular, pricePremium, description, isPremium) {
            ...
            document.getElementById('edit_price_regular').value = priceRegular;
            document.getElementById('edit_price_premium').value = pricePremium || '';
            document.getElementById('edit_description').value = description || '';
            document.getElementById('edit_is_premium').checked = isPremium;
        ...
        function confirmServiceDelete(event) {
