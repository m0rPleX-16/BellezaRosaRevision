...
@section('content')
    <div class="space-y-6" 
            ...
            <div class="flex flex-col sm:flex-row gap-3 items-end">
                <div class="flex-1">
                    <select id="dateFilter"
                        ...
                        <option value="this_month" {{ $currentFilter == 'this_month' ? 'selected' : '' }}>This Month
                        </option>
                        <option value="last_month" {{ $currentFilter == 'last_month' ? 'selected' : '' }}>Last Month
                        </option>
                        <option value="custom" {{ $currentFilter == 'custom' ? 'selected' : '' }}>Specific Month</option>
                        <option value="custom_range" {{ $currentFilter == 'custom_range' ? 'selected' : '' }}>Custom Range
                        </option>
                    </select>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <!-- Appointments -->
            <div class="card border-l-4 border-blue-500 cursor-pointer hover:shadow-md transition-shadow"
            ...
            <!-- Customers -->
            <div class="card border-l-4 border-green-500 cursor-pointer hover:shadow-md transition-shadow"
                onclick="viewCustomerServices()">
                <div class="flex">
            ...
            <!-- Staff -->
            <div class="card border-l-4 border-purple-500 cursor-pointer hover:shadow-md transition-shadow"
                onclick="viewStaff()">
                <div class="flex">
            ...
            <!-- Revenue -->
            <div class="card border-l-4 border-yellow-500 cursor-pointer hover:shadow-md transition-shadow"
                onclick="viewRevenue()">
                <div class="flex">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="card">
            <div class="flex items-center justify-between mb-4">
                ...
                <button id="toggleAppointmentsBtn" class="text-sm text-blue-600 hover:text-blue-800 font-medium hidden">
                ...
                @if (isset($rangeAppointments) && $rangeAppointments->count() > 0)
                    <div class="space-y-3" id="appointmentsContainer">
                        @foreach ($rangeAppointments->take(2) as $appointment)
                            <div
                                class="appointment-item flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                                <div class="flex items-center space-x-3">
                                    <div class="text-center bg-blue-100 rounded-lg p-2 min-w-16">
                                        <div class="font-bold text-blue-700">
                                            {{ $appointment->start_datetime->format('h:i') }}</div>
                                        <div class="text-xs text-blue-600">{{ $appointment->start_datetime->format('A') }}
                                        </div>
                                    </div>
                                    <div>
    ...
    {{ $appointment->status == 'no_show' ? 'text-gray-500' : '' }}">
                        ...
                        @if ($rangeAppointments->count() > 2)
                            @foreach ($rangeAppointments->skip(2) as $appointment)
                                <div
                                    class="appointment-item hidden flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
                                    <div class="flex items-center space-x-3">
                                        <div class="text-center bg-blue-100 rounded-lg p-2 min-w-16">
                                            <div class="font-bold text-blue-700">
                                                {{ $appointment->start_datetime->format('h:i') }}</div>
    ...
    <script>
        ...
        function updateDashboard(data) {
            ...
            if (data.appointments && data.appointments.length > 0) {
                ...
                data.appointments.forEach((appointment, index) => {
                    ...
                    const time12 = startTime.toLocaleTimeString('en-US', {
                        hour: 'numeric',
                        minute: '2-digit',
                        hour12: true
                    });
            ...
            `;
                    ...
                    `;
                        ...
                        if (customer.appointments.length > 3) {
                            tableHtml +=
                                `<div class="text-xs text-blue-600">+${customer.appointments.length - 3} more services</div>`;
                        }
                        tableHtml += '</div>';
                    } else {
                        tableHtml +=
        ...
        function showToast(message, type = 'success') {
            ...
            if (!toast) {
                ...
                toast.id = 'toast';
                toast.className =
                    'fixed bottom-4 right-4 px-6 py-3 rounded-lg shadow-lg z-50 transition-all duration-300 transform translate-y-10 opacity-0';
