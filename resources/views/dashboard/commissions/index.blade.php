...
@section('content')
    <div class="space-y-6">
        ...
        <!-- Filters -->
        <div class="card">
            <form action="{{ route('dashboard.commissions.index') }}" method="GET"
                class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex flex-col md:flex-row gap-4 flex-1">
                    <!-- Staff Filter -->
                    <select name="staff_id"
                        class="px-4 py-2 border border-gray-300 rounded-xl focus:ring-4 focus:ring-blue-200 focus:border-blue-600 outline-none w-full md:w-48">
                        <option value="">All Staff</option>
                        @foreach ($staff as $staffMember)
                            <option value="{{ $staffMember->id }}"
                                {{ request('staff_id') == $staffMember->id ? 'selected' : '' }}>
                    ...
                    <!-- Year Filter -->
                    <input type="number" name="year" value="{{ request('year', date('Y')) }}" min="2020"
                        max="{{ date('Y') + 5 }}"
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex space-x-2">
                                        ...
                                        @if ($commission->status == 'pending')
                                            <form action="{{ route('dashboard.commissions.pay', $commission) }}"
