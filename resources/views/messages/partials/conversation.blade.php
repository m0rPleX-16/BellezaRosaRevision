            ...
            @if ($appointments->count() > 0)
                <div class="relative">
                    ...
                    <div id="appointmentDropdown"
                        class="hidden absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg z-10 border border-gray-200">
                        <div class="p-3">
    ...
    @foreach ($messages as $message)
        <div class="flex {{ $message->sender_id == $currentUser->id ? 'justify-end' : 'justify-start' }}">
            <div class="max-w-xs lg:max-w-md">
                <div
                    class="rounded-2xl px-4 py-2 {{ $message->sender_id == $currentUser->id ? 'bg-blue-600 text-white' : 'bg-white border border-gray-200 text-gray-900' }}">
                    <p class="text-sm">{{ $message->message }}</p>
                </div>
                <div
                    class="text-xs text-gray-500 mt-1 {{ $message->sender_id == $currentUser->id ? 'text-right' : '' }}">
...
<script>
    ...
    document.addEventListener('click', function(event) {
        const dropdown = document.getElementById('appointmentDropdown');
        if (dropdown && !dropdown.contains(event.target) && !event.target.closest(
                'button[onclick="toggleAppointmentDropdown()"]')) {
